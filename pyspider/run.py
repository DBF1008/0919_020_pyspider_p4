...
def cli(ctx, **kwargs):
    ...
    # graceful shutdown: on SIGTERM/SIGINT, ask all components to finish
    # current tasks and quit
    if not ctx.obj.get('testing_mode'):
        def _quit_instances():
            for each in ctx.obj.get('instances', []):
                try:
                    ...
                    pass
        utils.install_graceful_shutdown(
            _quit_instances, logging.getLogger('run'))
...
@click.option('--metrics-host', default='0.0.0.0',
              help='metrics http server bind host')
@click.option('--metrics-port', envvar='SCHEDULER_METRICS_PORT', default=0,
              help='metrics http server port, exposes /metrics and /health, 0 to disable')
@click.pass_context
...
@click.option('--metrics-host', default='0.0.0.0',
              help='metrics http server bind host')
@click.option('--metrics-port', envvar='FETCHER_METRICS_PORT', default=0,
              help='metrics http server port, exposes /metrics and /health, 0 to disable')
@click.pass_context
...
@click.option('--metrics-host', default='0.0.0.0',
              help='metrics http server bind host')
@click.option('--metrics-port', envvar='PROCESSOR_METRICS_PORT', default=0,
              help='metrics http server port, exposes /metrics and /health, 0 to disable')
@click.pass_context
